...
function exnet_auto_fetch() {
    ...
    if(isset($_POST['exnet_sb'])) {
    	if(isset($_POST['exnet_url'])) {
    		$url = trim(strip_tags($_POST['exnet_url']));
    		if(stristr($url, 'imdb.com')) {
    			...
    			if(count($movie_data) AND !isset($movie_data['error'])) {
					...
					if($_POST['exnet_kind'] != 0)
						wp_set_post_terms( $post_id, (int) $_POST['exnet_kind'], 'watch-by-type');

					//add imdb url
					add_post_meta( $post_id, 'exnet_imdb_url', $url );

					//add release data
					add_post_meta( $post_id, 'exnet_release', $movie_data['release_date'] );

					//add runtime
					add_post_meta( $post_id, 'exnet_runtime', $movie_data['runtime'] . ' mins');

					//add initial meta rating & featured
					add_post_meta( $post_id, 'exnet_rating', 'N/A');
					add_post_meta( $post_id, 'exnet_featured', 'no');

					//add actors
					add_post_meta( $post_id, 'exnet_actors', implode(", ", $movie_data['cast']) );
    ...
    <form method="POST">
    	<input type="text" name="exnet_url" size="50" placeholder="imdb url">
    	<select name="exnet_kind">
    	...
    	<input type="submit" name="exnet_sb" class="button-primary" value="Fetch" />
	</form>
