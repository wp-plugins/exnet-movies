	...
	if(isset($_POST['id']) AND isset($_POST['value'])) {
		...
		if( $id > 0 ) {
			if($value == 'ok' OR $value == 'broken') {
				$cookie_link_rated = sprintf('themo_link_rated_%d_%d', $id, $ip);
				...
				if(get_option($cookie_link_rated)) {
					...
					$wpdb->query(
						$wpdb->prepare("UPDATE themo_links 
...
//add_shortcode( 'themo_star_rating', 'themo_rate' );
...
function themo_ajax() {
	...
	if(isset($_POST)) {
		...
		if(isset($_POST['themo_keyword']) AND trim(strip_tags($_POST['themo_keyword'])) != "") {
			global $wpdb;

			$title = '%' . trim(strip_tags($_POST['themo_keyword'])) . '%';
		...
		/* add genre */
		if(isset($_POST['themo-genre']) AND ($_POST['themo-genre'] != 'all')) {
			$args['tax_query'] = 
				array('relation' => 'AND', 
					array('taxonomy' => 'genre',
						 'field' => 'slug',
						  'terms' => array( $_POST['themo-genre'] )));
		}

		/* add actor */
		if(isset($_POST['themo_actor']) AND trim(strip_tags($_POST['themo_actor'])) != "")  {
			$args['meta_query'] = array(
									array('key' => 'themo_actors',
								    'value' => trim(strip_tags($_POST['themo_actor'])),
								    'compare' => 'LIKE'));
		}
...
add_filter( 'page_template', 'themo_page_template' );
function themo_page_template( $page_template )
{
    if ( is_page( 'themo-ajax' ) ) {
        $page_template = dirname( __FILE__ ) . '/blank-ajax-template.php';
    }

    if ( is_page( 'themo-redirect' ) ) {
        $page_template = dirname( __FILE__ ) . '/blank-ajax-template.php';
    }
