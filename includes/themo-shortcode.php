...
add_filter('posts_orderby','themo_orderby_linkscount');
function themo_orderby_linkscount( $arg ) {
...
add_filter('posts_fields', 'themo_fields');
function themo_fields($fields){
	global $wp_query, $wpdb, $use_linkcount_flag;

	if($use_linkcount_flag) $fields .= ", (SELECT COUNT(*) FROM themo_links WHERE mID = ID) AS link_count";
...
function themo_generate_tabs() {
	...
	$args['posts_per_page'] = THEMO_HOMEPAGE_PER_PAGE;
	    ...
	    foreach ( $terms as $term ) {
			...
			if($tab == 'top-rated') {
		      	$args['meta_query'] = array('relation' => 'AND',
										array('key' => 'themo_rating',
		                					   'value' => 'N/A',
		                						'compare' => '!='));

		      	$args['meta_key'] = 'themo_rating'; 
    		...
    		echo '</div>';//themo-small-item
	    	if($i%THEMO_ITEMS_PER_ROW==0) echo '<div style="clear:both;height:15px;"></div>';
...
add_shortcode('themo_generate_tabs', 'themo_generate_tabs');