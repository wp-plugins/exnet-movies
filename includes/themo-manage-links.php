...
function themo_manage_links() {
	...
	<div class="wrap">
		...
		<h2>Movie Links (<a href="edit.php?post_type=watch&amp;page=themo-links&amp;do=new">Add new</a>)</h2>
		...
		if(isset($_GET['act']) AND ($_GET['act'] == 'remove') AND isset($_GET['id'])) {
			$wpdb->delete( 'themo_links', array( 'linkID' => (int) $_GET['id'] ), array( '%d' ) );
			echo '<div class="updated bellow-h2">Link removed</div>';
		}

		if(isset($_GET['act']) AND ($_GET['act'] == 'approve') AND isset($_GET['id'])) {
			$wpdb->update( 'themo_links', array('status' => 'approved'), array( 'linkID' => (int) $_GET['id'] ) );
			echo '<div class="updated bellow-h2">Link approved</div>';
		}

		if(isset($_POST['movie-filter-id'])) 
			$movie_links = $wpdb->get_results($wpdb->prepare("SELECT * FROM themo_links WHERE mID = %d ORDER BY linkID DESC", (int) $_POST['movie-filter-id']));
		else
			$movie_links = $wpdb->get_results("SELECT * FROM themo_links ORDER BY linkID DESC");
		...
		<table class="wp-list-table widefat fixed posts">
			...
			<tbody>
				<?php foreach($movie_links as $l) : ?>
					<?php
					$status = ($l->status == 'pending') ? "Pending => <a href='edit.php?post_type=watch&amp;page=themo-links&amp;act=approve&amp;id=".$l->linkID."'>Approve</a>"  : 'Live';
					...
					<tr>
						...
						<td><?=$status?></td>
						<td><br />
							<a href="edit.php?post_type=watch&amp;page=themo-links&amp;do=update&amp;id=<?=$l->linkID?>">Edit</a><br />
							<a href="edit.php?post_type=watch&amp;page=themo-links&amp;act=remove&amp;id=<?=$l->linkID?>">Remove</a>
						</td>
					</tr>
				<?php endforeach; ?>
			</tbody>
		...
		elseif (isset($_GET['do'])) :
			if($_GET['do'] == 'new') themo_add_link();
			if($_GET['do'] == 'update' && isset($_GET['id'])) themo_update_link((int) $_GET['id']);
		endif;
		?>
...
function themo_add_link() {
	global $wpdb;

	echo '<h2>New link</h2>';

	if(isset($_POST['wpwpm-sb-link'])) {
		if($_POST['mID'] > 0 AND $_POST['link_tab'] != "" AND $_POST['link_title'] != "" AND $_POST['link_destination'] != "") {
			unset($_POST['wpwpm-sb-link']);
			$_POST['status'] = 'approved';

			if($wpdb->insert('themo_links', $_POST)) {
	...
	<form method="post">
		...
		<br/>
		<input type="submit" class="btn btn-medium btn-info" value="Save" name="wpwpm-sb-link"/>
	</form>
...
function themo_update_link($id) {
	...
	if(isset($_POST['wpwpm-sb-link'])) {
		if($_POST['mID'] > 0 AND $_POST['link_tab'] != "" AND $_POST['link_title'] != "" AND $_POST['link_destination'] != "") {
			...
			if($wpdb->update('themo_links', $_POST, array('linkID' => $id))) {
				echo '<div class="updated bellow-h2">Link saved successfully</div>';	
			}else{
				echo '<div class="updated bellow-h2">WP DB Error!</div>';
				$wpdb->show_errors();
	...
	$the_link = $wpdb->get_row($wpdb->prepare("SELECT * FROM themo_links WHERE linkID = %d", $id));
