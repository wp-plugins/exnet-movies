...
function exnet_all_kinds( $content ) {
	...
	?>
	<form id="exnet-filter" action="<?= bloginfo('url') ?>/exnet-ajax" method="POST" style="display:none;">
	...
	<?php elseif(get_query_var('genre')): ?>
		<input type="hidden" name="exnet-genre" value="<?=esc_html($fetch)?>" />
	<?php endif; ?>
	</form>
	<?php
...
add_shortcode( 'exnet_all_kinds', 'exnet_all_kinds' );


function exnet_filter_title( $title ) {
...
add_filter('wp_title', 'exnet_filter_title');